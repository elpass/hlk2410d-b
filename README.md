...
3.2 安装方式
手动安装：
将 hlk2410d_b 文件夹复制到 HA 的 custom_components/ 目录下
确保目录结构如下：
...
进入 设置 → 设备与服务 → 添加集成
搜索 HLK-LD2410D-B
选择扫描到的雷达设备（名称通常为 HLK-LD2410D-B 或类似）
完成配对，设备将自动添加到 HA
四、 实体说明
集成添加成功后，会自动创建以下实体：
4.1 传感器（Sensor）
...
确保蓝牙连接正常（诊断传感器显示"正常模式"）
点击 "读取" 按钮
观察 诊断 传感器的进度变化：10% → 30% → 60% → 90% → 100%
进度完成后，各数值输入实体会自动更新为模块当前值
5.2 修改参数：设置流程
在数值输入实体中调整所需参数（如将最大距离改为 5.0m）
点击 "设置" 按钮
诊断传感器会显示配置进度，仅修改的参数会被写入
完成后自动退出配置模式，恢复正常距离监测
5.3 重启模块
